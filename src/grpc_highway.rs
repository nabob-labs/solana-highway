//! Highway GRPC implementation that handles transaction routing between:
//! cascade-router -> highway-gateway -> highway
//!
...
use {
    crate::{
        config::ConfigHighwayGatewayClient,
        feature_flags::FeatureSet,
        metrics::{
            self,
            highway::{increment_send_transaction_error, increment_send_transaction_success},
        },
        payload::{TransactionDecoder, TransactionPayload},
        proto::highway::{
            auth_request, auth_response, highway_gateway_client::HighwayGatewayClient,
...
pub const DEFAULT_LOCK_KEY: &str = "highway-gateway";
...
async fn get_highway_gw_subscribe_auth_token(
    signer: ArcSigner,
    endpoint: String,
    ...
    let channel = Endpoint::from_shared(endpoint.clone())?
        ...
        .connect()
        .await?;
    let mut client = HighwayGatewayClient::with_interceptor(channel, x_token);
...
/// Establish a bidirectional streaming connection to the highway-gateway
///
...
pub async fn grpc_subscribe_highway_gw(
    signer: ArcSigner,
    endpoint: String,
    x_token: GrpcClientXToken,
    ...
    let otak =
        get_highway_gw_subscribe_auth_token(Arc::clone(&signer), endpoint.clone(), x_token.clone())
            .await?;
    ...
    async fn grpc_subscribe(
        ...
        loop {
            ...
            .await
            {
                ...
                Err(error) => {
                    error!(?error, "failed to connect to gRPC highway-gateway");
                        ...
                        tokio::select! {
                            _ = limit_interval.tick() => {
                                let messages_per100ms = config
                                    .max_streams
                                    .unwrap_or_else(metrics::highway::cluster_identity_stake_get_max_streams);
                                            ...
                                            match handler.handle_transaction(transaction).await {
                                                Ok(_) => {
                                                    if let Some(gateway_timestamp) = timestamp {
                                                        ...
                                                        metrics::highway::observe_forwarded_txn_latency(latency as f64);
                                                    }
        ...
        loop {
            match tasks.join_next().await {
                Some(Ok((Ok((sink, stream)), endpoint))) => {
                    info!(endpoint, "highway connected to gateway");
                    metrics::highway::gateway_set_connected(endpoints, endpoint);
