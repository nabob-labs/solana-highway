...
struct Config {
    ...
    /// Transactions output, Highway-like or Kafka
    #[serde(default)]
...
impl Default for ConfigOutput {
    fn default() -> Self {
        Self::Highway {
            highway: "http://127.0.0.1:8000/".to_owned(),
        }
    }
}
...
impl TransactionSender {
    async fn from_config(config: ConfigOutput) -> anyhow::Result<Self> {
        Ok(match config {
            ConfigOutput::Highway { highway } => Self::Highway {
                rpc: RpcClient::new(highway),
            },
            ConfigOutput::HighwayGateway { gateway } => {
                ...
                let mut client = HighwayGatewayClient::new(channel);
                ...
                Self::HighwayGateway { tx: Mutex::new(tx) }
            }
        })
    }
}
...
impl TransactionSender {
    async fn send(
        ...
        match self {
            Self::Highway { rpc } => rpc
                .send_transaction_with_config(&transaction, config)
                .await
                .map_err(Into::into),
            Self::HighwayGateway { tx } => {
